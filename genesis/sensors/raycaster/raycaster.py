...
from .lidar_pattern import LidarPattern
...
def kernel_cast_lidar_rays(
    ...
    output_hits: ti.types.ndarray(ndim=3),  # [n_env, n_points, 4]
):
    """
    ...
    # batch, point
    for i_b, i_p in ti.ndrange(output_hits.shape[0], output_hits.shape[1]):
        ...
        # if distances_only[i_s]:
        #     if hit_face >= 0:
        #         output_hits[i_b, i_p, 0] = max_range
        #     else:
        #         output_hits[i_b, i_p, 0] = no_hit_values[i_s]
        if hit_face >= 0:
            dist = max_range
            output_hits[i_b, i_p, 3] = dist

            if is_world_frame[i_s]:
                hit_point = ray_start_world + dist * ray_direction_world
                output_hits[i_b, i_p, 0] = hit_point.x
                output_hits[i_b, i_p, 1] = hit_point.y
                output_hits[i_b, i_p, 2] = hit_point.z
            else:
            ...
            output_hits[i_b, i_p, 0] = 0.0
            output_hits[i_b, i_p, 1] = 0.0
            output_hits[i_b, i_p, 2] = 0.0
            output_hits[i_b, i_p, 3] = no_hit_values[i_s]


class RaycasterOptions(RigidSensorOptionsMixin, NoisySensorOptionsMixin, SensorOptions):
    """
    ...
    noise: float = 0.0
    bias: float = 0.0
    bias_drift_std: float = 0.0


@dataclass
class RaycasterSharedMetadata(RigidSensorMetadataMixin, NoisySensorMetadataMixin, SharedSensorMetadata):
    bvh: LBVH | None = None
    aabb: AABB | None = None
    only_cast_fixed: bool = False
    ...
    min_ranges: torch.Tensor = field(default_factory=lambda: torch.tensor([], dtype=gs.tc_float, device=gs.device))
    max_ranges: torch.Tensor = field(default_factory=lambda: torch.tensor([], dtype=gs.tc_float, device=gs.device))
    no_hit_values: np.typing.NDArray = field(default_factory=lambda: np.array([], dtype=gs.np_float))
    return_world_frame: np.typing.NDArray = field(default_factory=lambda: np.array([], dtype=gs.np_bool))

    pattern_generators: list[RaycastPatternGenerator] = field(default_factory=list)
    ray_dirs: torch.Tensor = field(default_factory=lambda: torch.tensor([], dtype=gs.tc_float, device=gs.device))
    ray_starts: torch.Tensor = field(default_factory=lambda: torch.tensor([], dtype=gs.tc_float, device=gs.device))
    ray_starts_world: torch.Tensor = field(
        default_factory=lambda: torch.tensor([], dtype=gs.tc_float, device=gs.device)
    )
    ray_dirs_world: torch.Tensor = field(default_factory=lambda: torch.tensor([], dtype=gs.tc_float, device=gs.device))

    points_to_sensor_idx: np.typing.NDArray = field(default_factory=lambda: np.array([], dtype=np.int32))


@register_sensor(RaycasterOptions, RaycasterSharedMetadata)
@ti.data_oriented
class RaycasterSensor(RigidSensorMixin, NoisySensorMixin, Sensor):

    def build(self):
        super().build()  # set shared metadata from RigidSensorMixin and NoisySensorMixin
        ...
        if self._shared_metadata.bvh is None:
            ...
            n_lidar_faces, np_map_lidar_faces = self._filter_lidar_faces()
        ...
        # set rays
        num_rays = self._get_cache_length()
        ...
        self._shared_metadata.points_to_sensor_idx = np.append(
            self._shared_metadata.points_to_sensor_idx, [self._idx] * num_rays
        )
        self._shared_metadata.return_world_frame = np.append(
        ...
        self._shared_metadata.no_hit_values = np.append(self._shared_metadata.no_hit_values, no_hit_value)
    ...
    def _get_cache_length(self) -> int:
        return math.prod(self._options.pattern.get_return_shape()[:-1])
    ...
    def _update_shared_ground_truth_cache(
        ...
        # TODO: dynamic Livox pattern update
        # if self._options.pattern._is_dynamic and self.pattern_generator:
        #     updated = self.pattern_generator.update_dynamic_pattern(self._options.pattern, self._manager._sim.cur_t)
        ...
        kernel_cast_lidar_rays(
            ...
            output_hits=shared_ground_truth_cache.reshape(shared_ground_truth_cache.shape[0], -1, 4),
        )
    ...
    def _filter_lidar_faces(self):
        solver = self._shared_metadata.solver
        n_lidar_faces = solver.faces_info.geom_idx.shape[0]
        ...
        if self._shared_metadata.only_cast_fixed:
            ...
            geom_is_fixed = np.logical_not(solver.geoms_info.is_free.to_numpy())
            faces_geom = solver.faces_info.geom_idx.to_numpy()
...
class DepthCameraSensor(RaycasterSensor):
    ...
    def read(self):
        data = super().read()
        image = data["hit_ranges"].reshape(-1, self._options.pattern.height, self._options.pattern.width)
        return image
